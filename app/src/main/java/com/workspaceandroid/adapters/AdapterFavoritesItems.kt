...
class AdapterFavoritesItems(private val items : List<FavoritesResponse>, val context: Context) : RecyclerView.Adapter<ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        return ViewHolder(LayoutInflater.from(context).inflate(R.layout.item_favorite_row, parent, false))
    }
...
class ViewHolder (view: View) : RecyclerView.ViewHolder(view) {
    val tvFavoriteTitle: TextView = view.tv_favorite_title
    val ivFavoritePicture: ImageView = view.iv_favorite_picture
}